...
ICs = [f'IC{i}' for i in range(1, 56)]
D = len(ICs)

# Step 2: Generate all combinations: 'ICj' + 'ICi' 
combs = [f"FC100 ({i}-{j})" for j, i in product(ICs, ICs)]

# Step 3: Reshape to matrix
ICs_matrix = np.array(combs).reshape(D, D)

# Step 4: Extract upper triangle COLUMNWISE (to match MATLAB)
ICs_vector = ICs_matrix.T[np.triu_indices(D, k=1)]
...
with open("/Users/baileyng/MIND_models/miscellaneous/FC100_colnames.txt", 'w') as f:
