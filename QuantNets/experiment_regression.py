...
class ExperimentRegression:
  ...
  def __init__(self, 
    ...
    if image_to_graph_supported:
      ...
      # load in the dataset ...
      data_struct = read_cached_graph_dataset(num_train=num_train, num_test=num_test, dataset_name=dataset_name, parent_dir = base_path)
    ...
    if base_path == None:
      ...
      if id == None:
        ExperimentRegression.experiment_id += 1 # increment the id ...
      else:
  ...
  def __cache_results(self, train_cnn_loss_array, train_qgcn_loss_array, train_sgcn_loss_array, 
                      train_cnn_mse_array, train_qgcn_mse_array, train_sgcn_mse_array,
                      test_cnn_mse_array, test_qgcn_mse_array, test_sgcn_mse_array):
    # define the paths to save the results ...
    if self.cnn_specific_run_dir != None:
      train_cnn_loss_filepath = os.path.join(self.cnn_specific_run_dir, "train_loss.pk")
      train_cnn_mse_filepath  = os.path.join(self.cnn_specific_run_dir, "train_mse.pk")
      test_cnn_mse_filepath   = os.path.join(self.cnn_specific_run_dir, "test_mse.pk")
      with open(train_cnn_loss_filepath, 'wb') as train_cnn_loss_file:
          pickle.dump(train_cnn_loss_array, train_cnn_loss_file)
      with open(train_cnn_mse_filepath, 'wb') as train_cnn_mse_file:
          pickle.dump(train_cnn_mse_array, train_cnn_mse_file)
      with open(test_cnn_mse_filepath, 'wb') as test_cnn_mse_file:
          pickle.dump(test_cnn_mse_array, test_cnn_mse_file)
    if self.qgcn_specific_run_dir != None:
      ...
      with open(train_qgcn_mse_filepath, 'wb') as train_qgcn_mse_file:
          pickle.dump(train_qgcn_mse_array, train_qgcn_mse_file)
      with open(test_qgcn_mse_filepath, 'wb') as test_qgcn_mse_file:
          pickle.dump(test_qgcn_mse_array, test_qgcn_mse_file)
    if self.sgcn_specific_run_dir != None:
      ...
      with open(train_sgcn_mse_filepath, 'wb') as train_sgcn_mse_file:
          pickle.dump(train_sgcn_mse_array, train_sgcn_mse_file)
      with open(test_sgcn_mse_filepath, 'wb') as test_sgcn_mse_file:
          pickle.dump(test_sgcn_mse_array, test_sgcn_mse_file)


  def load_cached_results(self):
    train_cnn_loss_array, train_qgcn_loss_array, train_sgcn_loss_array = None, None, None
    train_cnn_mse_array, train_qgcn_mse_array, train_sgcn_mse_array = None, None, None
    test_cnn_mse_array, test_qgcn_mse_array, test_sgcn_mse_array = None, None, None
    ...
    if self.cnn_specific_run_dir != None and os.path.exists(self.cnn_specific_run_dir):
      ...
      with open(train_cnn_mse_filepath, 'rb') as train_cnn_mse_file:
          train_cnn_mse_array = pickle.load(train_cnn_mse_file)
      with open(test_cnn_mse_filepath, 'rb') as test_cnn_mse_file:
          test_cnn_mse_array = pickle.load(test_cnn_mse_file)
    if self.qgcn_specific_run_dir != None and os.path.exists(self.qgcn_specific_run_dir):
      ...
      with open(train_qgcn_mse_filepath, 'rb') as train_qgcn_mse_file:
          train_qgcn_mse_array = pickle.load(train_qgcn_mse_file)
      with open(test_qgcn_mse_filepath, 'rb') as test_qgcn_mse_file:
          test_qgcn_mse_array = pickle.load(test_qgcn_mse_file)
    if self.sgcn_specific_run_dir != None and os.path.exists(self.sgcn_specific_run_dir):
      ...
      with open(train_sgcn_mse_filepath, 'rb') as train_sgcn_mse_file:
          train_sgcn_mse_array = pickle.load(train_sgcn_mse_file)
      with open(test_sgcn_mse_filepath, 'rb') as test_sgcn_mse_file:
          test_sgcn_mse_array = pickle.load(test_sgcn_mse_file)
  ...
  def __train(self):
    ...
    if self.cnn_model_exists:
      ...
      for data in self.raw_train_dataloader:
        ...
        cnn_loss = F.mse_loss(cnn_output, y_data.float())
        cnn_loss.backward()
  ...
  def __evaluate(self, eval_train_data=False):
    ...
    if self.cnn_model_exists:
      ...
      for data in raw_dataset_loader:
        ...
        x_data = x_data.to(self.device)
        y_data = y_data.to(self.device)
        pred = self.cnn_model(x_data)
        mse = F.mse_loss(pred, y_data.float(), reduction='sum')
        total_mse += mse.item()
        total_samples += len(x_data)
      cnn_mse = total_mse / total_samples
      if self.profile_run: stop_time = time.time()
      if self.profile_run: profile_stats = f"; Epoch took a total of {stop_time - start_time}s"
      print(f"{'train' if eval_train_data else 'test'} data: cnn eval done{ profile_stats if self.profile_run else '' }")

    # perform the evaluation for the qgcn ...
    qgcn_mse = 0
    if self.qgcn_model_exists: 
      if eval_train_data:
    ...
    # perform the evaluation for the sgcn ...
    sgcn_mse = 0
    if self.sgcn_model_exists: 
      if eval_train_data:
    ...
    # output provided in the form -> cnn, qgcn, sgcn ...
    return cnn_mse, qgcn_mse, sgcn_mse
  ...
  def run(self, num_epochs=None, eval_training_set=True):
    ...
    for epoch in range(1, num_epochs):
        ...
        train_cnn_mse, train_qgcn_mse, train_sgcn_mse = 0, 0, 0
        if eval_training_set:
            train_cnn_mse, train_qgcn_mse, train_sgcn_mse = self.__evaluate(eval_train_data=True)
        train_cnn_mse_array.append(train_cnn_mse)
        train_qgcn_mse_array.append(train_qgcn_mse)
        train_sgcn_mse_array.append(train_sgcn_mse)
        test_cnn_mse,  test_qgcn_mse,  test_sgcn_mse = self.__evaluate(eval_train_data=False)
        test_cnn_mse_array.append(test_cnn_mse)
        ...
        train_mse_str = "CNN_Train_MSE: {:.5f}, QGCN_Train_MSE: {:.5f}, SGCN_Train_MSE: {:.5f}, ".format(train_cnn_mse, train_qgcn_mse, train_sgcn_mse)
        test_mse_str = "CNN_Test_MSE: {:.5f}, QGCN_Test_MSE: {:.5f}, SGCN_Test_MSE: {:.5f}, ".format(test_cnn_mse,  test_qgcn_mse,  test_sgcn_mse)
  ...
  # plots both MSE and losses on the same graph ...
  @staticmethod
