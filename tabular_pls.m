...
% Save PLS weights in structured format
weights_dir = '/Users/baileyng/MIND_models/models_plsregression/PLS_weights_matlab';
if ~exist(weights_dir, 'dir')
    mkdir(weights_dir);
end

% Create weights table with proper column names
W = stats.W; % X-weights matrix (features x components)

% Create component column names
component_names = cell(1, ncomp);
for i = 1:ncomp
    component_names{i} = sprintf('Component_%d', i);
end

% Create weights table
weights_table = array2table(W, ...
    'RowNames', regions, ...  % Use the region names as row names
    'VariableNames', component_names);
...
writetable(weights_table, weights_filepath, 'WriteRowNames', true);

fprintf('PLS weights saved to: %s\n', weights_filepath);
fprintf('Final model parameters: n_components=%d\n', ncomp);
fprintf('Weights matrix size: %d features x %d components\n', size(W, 1), size(W, 2));

% Display summary of weights
for comp = 1:ncomp
    fprintf('Component %d: mean=%.4f, std=%.4f, range=[%.4f, %.4f]\n', ...
        comp, mean(W(:,comp)), std(W(:,comp)), min(W(:,comp)), max(W(:,comp)));
end
